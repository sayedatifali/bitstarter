...
var checkHtmlFileFromUrl = function(url, checksfile) {
    var response2file = responseFromUrl(checksfile);
    rest.get(url).on('complete', response2file);
}

var responseFromUrl = function(checksfile) {
    ...
    var response2file = function(result, response) {
        if (result instanceof Error) {
            ...
            var value = checkHtmlFile(HTMLFILE_DEFAULT, checksfile);
            var outJson = JSON.stringify(value, null, 4);
            console.log(outJson);
        }
    };
...
if(require.main == module) {
    ...
    /* if (program.file) {
       ...
       console.log("Humba Hu!");
       } else {
       var checkJson = checkHtmlFileFromUrl(program.url, program.checks);
       }*/
    if(program.url) {
